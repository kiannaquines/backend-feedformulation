...
def get_ingredients(
    db: Session = Depends(get_db), auth_user: dict = Depends(verify_jwt_token)
):
    """
    ...
    if not ingredients:
        raise HTTPException(
            ...
            detail="Ingredients are currently empty",
        )
...
def create_ingredient(
    ...
    try:
        ...
        raise HTTPException(
            ...
            detail=f"An error occured while adding ingredient",
        )
    except Exception:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
@ingredient_router.delete(
    "/ingredients/delete/{ingredient_id}", status_code=status.HTTP_200_OK
)
...
def ai_chat_assistant(
    ...
    try:
        ...
        # Add created_at timestamp to response
